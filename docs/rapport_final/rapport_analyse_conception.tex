...
\usepackage{setspace}
\onehalfspacing
\usepackage{lscape}   % rotation en paysage
\usepackage{pdflscape} % paysage avec rotation PDF viewer
...
\usepackage{graphicx}
\usepackage{float}     % [H] placement strict
\usepackage{caption}
...
% Chemin vers les figures PDF converties
\graphicspath{{figures/}}

% Style des légendes : gras pour le label, italique pour le titre
...
\usepackage[hidelinks,
  ...
  pdfauthor={Groupe 5GI Depot 1},
  pdfsubject={Gestion de Flotte de Vehicules}]{hyperref}
...
\fancyhead[R]{\small\textcolor{fleetblue}{\textbf{FleetMan -- Rapport A\&C}}}
...
\newtcolorbox{remarque}[1][]{colback=fleetblue!5,colframe=fleetblue,
  fonttitle=\bfseries,title={Remarque},breakable,#1}
\newtcolorbox{definition}[2][]{colback=fleetdark!4,colframe=fleetdark,
  fonttitle=\bfseries\color{white},coltitle=white,title={#2},breakable,#1}
\newtcolorbox{lecture}[1][]{colback=fleetsuccess!5,colframe=fleetsuccess,
...
% --- Page de garde -----------------------------------------------------------
\begin{titlepage}
  \pagecolor{fleetdark}\color{white}\centering
  \vspace*{1.5cm}
  {\small ECOLE POLYTECHNIQUE DE YAOUNDE\\
   Departement Genie Informatique --- 5\textsuperscript{eme} Annee}\\[1.2cm]
  \textcolor{fleetblue}{\rule{0.8\textwidth}{2pt}}\\[0.5cm]
  {\fontsize{48}{58}\bfseries\textcolor{fleetblue}{FleetMan}}\\[0.3cm]
  {\Large Plateforme de Gestion de Flotte de Vehicules}\\[0.5cm]
  \textcolor{fleetblue}{\rule{0.8\textwidth}{1pt}}\\[1cm]
  {\LARGE\bfseries Rapport d'Analyse et de Conception}\\[0.3cm]
  {\large Architecture Hexagonale Reactive --- Backend Spring Boot}\\[1.5cm]
  \begin{tabular}{rl}
    \textcolor{white!70}{Theme :}     & \textbf{Gestion de Flotte Multi-Tenant} \\[0.3cm]
    \textcolor{white!70}{Stack :}     & \textbf{Spring Boot 3.2 $\cdot$ WebFlux $\cdot$ PostgreSQL $\cdot$ React} \\[0.3cm]
    \textcolor{white!70}{Promotion :} & \textbf{5GI --- Depot 1 --- Annee 2025--2026} \\[0.3cm]
    \textcolor{white!70}{Encadrant :} & \textbf{[Nom de l'encadrant]} \\
  \end{tabular}
  \vfill
  {\small Juin 2026}
\end{titlepage}
...
La gestion efficace d'une flotte de vehicules constitue un enjeu operationnel
et economique majeur pour les entreprises de transport en Afrique centrale.
Ce projet vise a concevoir et implementer \textbf{FleetMan}, une plateforme
B2B de gestion de flotte complete, deploye comme module metier de
l'ecosysteme RT-Comops.

\noindent\textbf{Materiel et methodes.}
Le backend est developpe en Java 21 sur Spring Boot 3.2 avec une architecture
hexagonale reactive (WebFlux, R2DBC, PostgreSQL 16).
Le frontend est realise en Next.js 16 avec TypeScript et Tailwind CSS.
La methodologie adoptee est le Domain-Driven Design associe au pattern
Ports \& Adapters.

\noindent\textbf{Resultats.}
La plateforme expose \textbf{134 endpoints REST} documentes via Swagger/OpenAPI,
repartis en \textbf{20 modules fonctionnels}.
Dix migrations Liquibase structurent \textbf{35 tables PostgreSQL}.
La latence P95 cible est inferieure a 200\,ms.

\noindent\textbf{Discussion et conclusion.}
FleetMan constitue une reponse complete aux besoins de supervision de flotte
identifies. Son architecture hexagonale garantit une testabilite maximale
et une extractibilite vers des microservices sans refactorisation du domaine.

\medskip
\noindent\textbf{Mots-cles :} gestion de flotte, architecture hexagonale,
Spring WebFlux, multi-tenant, georeperage, Domain-Driven Design, R2DBC.
...
Ce rapport est organise en cinq parties~:
\textbf{(I)} Etat de l'art et analyse de l'existant~;
\textbf{(II)} Analyse des besoins~;
\textbf{(III)} Conception de l'architecture~;
\textbf{(IV)} Implementationet resultats~;
\textbf{(V)} Integration au Kernel RT-Comops et perspectives.
...
\subsection{Flux d'authentification detaille}

Le processus d'authentification FleetMan suit une architecture a 3 niveaux~:

\begin{enumerate}[leftmargin=2cm]
  \item \textbf{Niveau API Gateway}~: Reception de la requete HTTP avec
        credentials (email + password) ou token JWT dans l'en-tete
        \texttt{Authorization: Bearer <token>}.
        
  \item \textbf{Niveau Service Auth externe}~: Delegation de la validation
        au service Auth centralisé du Kernel RT-Comops. Ce service valide
        les credentials, genere un JWT RS256 signe avec cle privee,
        et retourne les claims utilisateur (userId, tenantId, roles, permissions).
        
  \item \textbf{Niveau Application FleetMan}~: Stockage ou mise a jour
        de l'utilisateur local dans la table \texttt{users}, cache des
        permissions dans Redis (TTL 1h), injection du contexte de securite
        dans le ReactiveSecurityContextHolder pour les requetes subsequentes.
\end{enumerate}
...
\begin{lecture}
La validation du token se fait en deux etapes~: (1)~le \texttt{BearerTokenFilter}
extrait le token de l'en-tete HTTP \texttt{Authorization: Bearer <token>}
via une expression reguliere~; (2)~le \texttt{RemoteAuthAdapter}
appelle le service Auth externe pour valider le token et recuperer les
claims utilisateur. L'utilisateur local est cree ou mis a jour
dans \texttt{UserLocalRepository} apres validation, permettant la tracabilite
des actions dans les logs d'audit. Les permissions sont cached dans Redis
avec une cle \texttt{user:permissions:<userId>} et un TTL de 1 heure,
evitant des appels reseau repetitifs. En cas d'echec de validation (token
expire, signature invalide, utilisateur revoque), une exception
\texttt{InvalidTokenException} est levee et mappee en HTTP 401 Unauthorized
par le \texttt{GlobalExceptionHandler}.
\end{lecture}
...
% =============================================================================
\part{Implementation et Resultats}
% =============================================================================
...
\begin{table}[H]
\caption{Stack technique complete de FleetMan}
\label{tab:stack}
\centering\small
...
\midrule
Spring Boot WebFlux & 3.2.6    & Framework web reactif non bloquant \\
Java               & 21 (LTS) & Langage de programmation \\
PostgreSQL + PostGIS & 16     & Base de donnees avec extensions geometriques \\
R2DBC              & runtime  & Driver reactif PostgreSQL \\
Redis              & alpine   & Cache telemetrie et quotas \\
Apache Kafka       & 3.7.0    & Bus d'evenements (mode KRaft) \\
Liquibase          & 4.x      & Migrations de schema (10 changesets) \\
MapStruct          & 1.5.5    & Mapping objet genere \\
Lombok             & 1.18.30  & Reduction du boilerplate \\
SpringDoc OpenAPI  & 2.5.0    & Documentation Swagger/OpenAPI 3 \\
Resilience4j       & Cloud    & Circuit Breaker reactif \\
Micrometer + Prometheus & --  & Metriques et monitoring \\
Next.js            & 16.2.6   & Framework frontend (SSR) \\
...
\begin{table}[H]
\caption{Recapitulatif des endpoints REST par module (total : 134)}
\label{tab:endpoints}
\centering
\begin{tabularx}{\textwidth}{p{4.5cm}p{2cm}X}
\toprule
\textbf{Module} & \textbf{Nb} & \textbf{Chemin de base} \\
\midrule
Authentification      & 3   & \texttt{/api/v1/auth} \\
Vehicules             & 11  & \texttt{/api/v1/vehicles} \\
Conducteurs           & 5   & \texttt{/api/v1/drivers} \\
Flottes               & 6   & \texttt{/api/v1/fleets} \\
Trajets               & 6   & \texttt{/api/v1/trips} \\
Georeperage           & 11  & \texttt{/api/v1/geofence} \\
Maintenance           & 8   & \texttt{/api/v1/operations/maintenances} \\
Incidents             & 12  & \texttt{/api/v1/operations/incidents} \\
Carburant             & 8   & \texttt{/api/v1/operations/fuel-recharges} \\
Planification         & 9   & \texttt{/api/v1/schedules} \\
Affectations          & 13  & \texttt{/api/v1/assignments} \\
Documents vehicules   & 5   & \texttt{/api/v1/vehicles/\{id\}/documents} \\
Documents conducteurs & 5   & \texttt{/api/v1/drivers/\{id\}/documents} \\
Conformite            & 3   & \texttt{/api/v1/documents} \\
KPIs                  & 9   & \texttt{/api/v1/kpis} \\
Administration        & 20  & \texttt{/api/v1/admin, /api/v1/super-admin} \\
\midrule
...
% =============================================================================
\part{Integration au Kernel RT-Comops}
% =============================================================================

\chapter{Integration au Kernel RT-Comops}

\section{Modules Kernel applicables a FleetMan}

\begin{table}[H]
\caption{Modules Kernel RT-Comops integres dans FleetMan}
\label{tab:kernel}
\centering
\begin{tabularx}{\textwidth}{p{3.5cm}p{2.5cm}X}
\toprule
\textbf{Module Kernel} & \textbf{Priorite} & \textbf{Apport a FleetMan} \\
\midrule
\texttt{kernel-core}       & Critique & Multi-tenancy, JWT RS256, audit immuable \\
\texttt{auth-core}         & Critique & Remplace l'auth locale, apporte MFA \\
\texttt{roles-core}        & Critique & RBAC granulaire avec cache Redis \\
\texttt{organization-core} & Haute    & Flotte = Organisation, depot = Agence \\
\texttt{resource-core}     & Haute    & Vehicules comme ressources materielles \\
\texttt{common-core}       & Haute    & BaseEntity, Address, Contact, PageResult \\
\texttt{file-core}         & Normale  & Documents legaux centralises \\
\texttt{tp-core}           & Normale  & Clients des missions comme tiers \\
\texttt{settings-core}     & Normale  & Numerotation et parametres metier \\
\bottomrule
\end{tabularx}
\end{table}

\section{Plan d'integration en 6 phases}

\begin{table}[H]
\caption{Plan d'integration Kernel dans FleetMan (16 semaines)}
\label{tab:plan_kernel}
\centering
\begin{tabularx}{\textwidth}{p{1cm}p{4.5cm}p{2.5cm}p{2cm}X}
\toprule
\textbf{Phase} & \textbf{Modules} & \textbf{Semaines} & \textbf{Effort} & \textbf{Livrable} \\
\midrule
0 & Enregistrement ClientApp  & 1--2   & Small  & FleetMan declare dans Kernel \\
1 & auth-core, roles-core     & 3--5   & Medium & JWT RS256 + RBAC granulaire \\
2 & organization-core         & 6--7   & Medium & Flottes synchronisees \\
3 & resource-core             & 8--10  & Large  & Vehicules dans le Kernel \\
4 & actor-core, tp-core       & 11--12 & Medium & Conducteurs et clients \\
5 & file-core, settings-core  & 13--14 & Medium & Documents et numerotation \\
6 & common-core, finalisation & 15--16 & Small  & Types alignes, tests e2e \\
\bottomrule
\end{tabularx}
\end{table}
...
Ce projet a conduit a la conception et a l'implementation de la plateforme
\textbf{FleetMan}, avec les principaux apports suivants~:

\begin{itemize}[leftmargin=2cm]
  \item Une architecture hexagonale reactive exposant
        \textbf{134 endpoints REST documentes}~;
  \item \textbf{20 modules fonctionnels} couvrant l'ensemble du cycle
        d'exploitation d'une flotte~;
  \item Un frontend Next.js 16 avec TypeScript et Tailwind CSS~;
  \item Un plan d'integration detaille avec le Kernel RT-Comops
        en 6 phases sur 16 semaines.
\end{itemize}

\section*{Limites}

\begin{itemize}[leftmargin=2cm]
  \item L'integration au Kernel RT-Comops est planifiee mais non encore
        implementee~;
  \item Les modules Depenses/Budget, Clients/Missions et Alertes
        configurables restent a implementer~;
  \item L'application mobile conducteur Android est hors perimetre.
\end{itemize}

\section*{Perspectives}

\begin{itemize}[leftmargin=2cm]
  \item Generation automatique de rapports PDF pour les KPIs~;
  \item Tableau de bord cartographique temps reel (WebSocket)~;
  \item Intelligence artificielle pour la prediction des maintenances~;
  \item Ancrage des incidents critiques dans le blockchain-core Kernel.
\end{itemize}
