...
public class AuthService implements AuthUseCase {
        ...
        public Mono<AuthPort.AuthResponse> login(
                ...
                return authPort
                                ...
                                .flatMap(response -> resolveLocalUser(response.user())
                                                .flatMap(localId -> ensureRoleProfileExistsForLocalId(response.user(),
                                                                localId)
                                                                .then(checkUserAccessByLocalId(localId)))
                                                .then(enrichWithLocalData(response.user()))
                                                .map(enrichedUser -> new AuthPort.AuthResponse(
        ...
        @Override
        public Mono<AuthPort.AuthResponse> selectContext(String selectionToken, String contextId, UUID organizationId) {
                return authPort.selectContext(selectionToken, contextId, organizationId)
                                .flatMap(response -> resolveLocalUser(response.user())
                                                .flatMap(localId -> ensureRoleProfileExistsForLocalId(response.user(),
                                                                localId)
                                                                .then(checkUserAccessByLocalId(localId)))
                ...
                return ensureRolesExist(command.roles())
                                ...
                                .onErrorResume(e -> {
                                        if (e instanceof AuthException ae &&
                                                ...
                                                // 409 Conflict
                                                return userRepo
                                                                .findByUsername(command.username()) // On cherche en
                                                                                                    // local
                                                                .flatMap(u -> {
                                                                        if (!u.isActive()) {
                                ...
                                .flatMap(res -> {
                                        ...
                                        if (token == null || token.isBlank()) {
                                                return resolveLocalUser(res.user())
                                                                .flatMap(localId -> ensureRoleProfileExistsForLocalId(
                                                                                res.user(), localId))
                                                                .thenReturn(res);
                                        }
                                        ...
                                        return photoFlow
                                                        ...
                                                        .flatMap(freshUser -> resolveLocalUser(freshUser)
                                                                        .flatMap(localId -> ensureRoleProfileExistsForLocalId(
                                                                                        freshUser, localId))
                                                                        .thenReturn(
        ...
        public Mono<AuthPort.UserDetail> me(String token) {
                return authPort
                                ...
                                .flatMap(remote -> resolveLocalUser(remote)
        ...
        /**
         * FONCTION CENTRALE : Garantit que Hassana voit ses managers/drivers en DB
         * locale.
         * Accepte optionnellement l'UUID local (peut différer de user.id() si merge
         * seed)
         */
                                ...
                                return managerPort
                                                ...
                                                .then(
                                                                userRepo
                                                                                ...
                                                                                .switchIfEmpty(Mono.defer(() -> userRepo
                                                                                                .findByKernelId(user
                                                                                                                .id())))
                                                                                .flatMap(localUser -> {
                                                                                        localUser.setActive(true);
                                                                                        localUser.setApprovalStatus(
                                                                                                        "APPROVED");
                                ...
                                .flatMap(user -> {
                                        if (user.roles().contains("FLEET_MANAGER")) {
                                                ...
                                                return userRepo.findByKernelId(userId)
                                                                .switchIfEmpty(Mono
                                                                                .defer(() -> userRepo.findById(userId)))
                                                                .flatMap(localUser -> fleetRepo
                                                                                .findAllByManagerId(localUser.getId())
                                                                                .hasElements()
                                                                                .flatMap(has -> has
                                                                                                ? Mono.<Void>error(
                                                                                                                new AuthException(
        ...
        /**
         * Sync le user en DB locale et retourne son UUID LOCAL (peut ≠ UUID Kernel si
         * merge seed).
         */
        private Mono<UUID> resolveLocalUser(AuthPort.UserDetail remote) {
                return userRepo.findByKernelId(remote.id())
                                ...
                                .flatMap(local -> {
                                        if (remote.username() != null && !remote.username().isBlank())
                                                local.setUsername(remote.username());
                                        if (remote.email() != null && !remote.email().isBlank())
                                                local.setEmail(remote.email());
                                        if (remote.firstName() != null && !remote.firstName().isBlank())
                                                local.setFirstName(remote.firstName());
                                        if (remote.lastName() != null && !remote.lastName().isBlank())
                                                local.setLastName(remote.lastName());
                                        if (remote.photoUrl() != null && !remote.photoUrl().isBlank())
                                ...
                                .switchIfEmpty(Mono.defer(() -> userRepo.findByEmail(remote.email())
                                                .flatMap(existingByEmail -> {
                                                        log.info("🔄 [SYNC] Merge email={} — local id={}, kernel_id={}",
                                                                        remote.email(), existingByEmail.getId(),
                                                                        remote.id());
                                                        ...
                                                        existingByEmail.setFirstName(
                                                                        remote.firstName() != null ? remote.firstName()
                                                                                        : existingByEmail
                                                                                                        .getFirstName());
                                                        existingByEmail.setLastName(
                                                                        remote.lastName() != null ? remote.lastName()
                                                                                        : existingByEmail
                                                                                                        .getLastName());
                                                        existingByEmail.setPhotoUrl(
                                                                        remote.photoUrl() != null ? remote.photoUrl()
                                                                                        : existingByEmail
                                                                                                        .getPhotoUrl());
                                                        existingByEmail.setLastLoginAt(Instant.now());
                                                        existingByEmail.setKernelId(remote.id());
                                                        existingByEmail.setNew(false);
                                                        return userRepo.save(existingByEmail)
                                                                        .map(saved -> saved.getId());
                                                })
                                                .switchIfEmpty(Mono.defer(() -> {
                                                        log.info("📝 [SYNC] Nouveau user : kernel_id={} email={}",
                                                                        remote.id(), remote.email());
        ...
        private Mono<AuthPort.UserDetail> enrichWithLocalData(
                ...
                return userRepo.findByKernelId(remote.id())
                                .switchIfEmpty(Mono.defer(() -> userRepo.findById(remote.id())))
                                .flatMap(localUser -> {
                                        String finalFirstName = (remote.firstName() != null
                                                        && !remote.firstName().isBlank()) ? remote.firstName()
                                                                        : localUser.getFirstName();
                                        String finalLastName = (remote.lastName() != null
                                                        && !remote.lastName().isBlank()) ? remote.lastName()
                                                                        : localUser.getLastName();
                                        String finalPhotoUrl = (remote.photoUrl() != null
                                                        && !remote.photoUrl().isBlank()) ? remote.photoUrl()
                                                                        : localUser.getPhotoUrl();

                                        if (remote.roles().contains("FLEET_MANAGER")) {
                                                return managerPort
                                                                .getCompanyName(localUser.getId())
                                                                .map(c -> new AuthPort.UserDetail(
                                                                                ...
                                                                                remote.isActive(),
                                                                                remote.lastLoginAt()))
                                                                .defaultIfEmpty(new AuthPort.UserDetail(
                                                                                remote.id(),
                                                                                remote.username(),
                                                                                remote.email(),
                                                                                remote.phone(),
                                        ...
                                        return Mono.just(new AuthPort.UserDetail(
                                                        remote.id(),
                                                        remote.username(),
                                                        remote.email(),
                                                        remote.phone(),
