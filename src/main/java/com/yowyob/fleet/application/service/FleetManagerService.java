...
public class FleetManagerService implements ManageFleetManagerUseCase {
        ...
        private Mono<FleetManagerResponse> syncAndEnrich(AuthPort.UserDetail remoteUser) {
                return Mono.zip(
                                managerRepository.findById(remoteUser.id())
                                                .switchIfEmpty(managerPersistencePort
                                                                .createProfile(remoteUser.id(),
                                                                                "Société de " + remoteUser.lastName())
                                                                .then(managerRepository.findById(remoteUser.id()))),
                                fleetRepository.countByManagerId(remoteUser.id())).map(tuple -> {
                                        ...
                                        return new FleetManagerResponse(
                                                        ...
                                                        fleetCount.intValue(),
                                                        remoteUser.photoUrl(),
                                                        localEntity.getPhone() != null ? localEntity.getPhone() : "",
                                                        localEntity.getAddress() != null ? localEntity.getAddress()
                                                                        : "",
                                                        localEntity.getCity() != null ? localEntity.getCity() : "",
                                                        localEntity.getLogoUrl() != null ? localEntity.getLogoUrl()
                                                                        : "");
                                });
        }

        @Override
        public Mono<Void> updateManagerCompany(UUID userId, String companyName, String phone, String address,
                        String city,
                        String logoUrl) {
                if (companyName == null || companyName.trim().isEmpty()) {
                        return Mono.error(ManagerException.invalidCompanyData("Le nom ne peut pas être vide."));
                }
                return managerPersistencePort.updateCompanyDetails(userId, companyName, phone, address, city, logoUrl);
        }
                ...
                return Mono.zip(
                                ...
                                maintenanceRepository.findAllByManagerId(managerId)
                                                .filter(m -> m.getDateTime() != null
                                                                && m.getDateTime().isAfter(startOfMonth))
                                                .count(),
                        ...
                        Mono<BigDecimal> totalFuelLitersMono = fuelRechargeRepository.findAllByManagerId(managerId)
                                        .filter(fr -> fr.getRechargeDateTime() != null
                                                        && fr.getRechargeDateTime().isAfter(startOfMonth))
        ...
        @Override
        public Mono<FleetManagerPersistencePort.CompanyDetails> getOrganizationDetails(UUID userId) {
                return managerPersistencePort.getCompanyDetails(userId);
        }

        @Override
        public Mono<Void> updateOrganizationDetails(UUID userId, FleetManagerPersistencePort.CompanyDetails details) {
                return managerPersistencePort.updateCompanyDetails(
                                userId,
                                details.companyName(),
                                details.phone(),
                                details.address(),
