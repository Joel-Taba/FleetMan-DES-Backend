...
public class TripService implements ManageTripUseCase {
                                                        ...
                                                        ? operationalRepo
                                                                        ...
                                                                        .flatMap(op -> {
                                                                                double cur = op.getOdometerReading() != null
                                                                                                ? op.getOdometerReading()
                                                                                                                .doubleValue()
                                                                                                : 0;
                                                                                op.setOdometerReading(
                                                                                                BigDecimal.valueOf(cur
                                                                                                                + dist.doubleValue()));
                                                                                op.setMileage(
                                                                                                BigDecimal.valueOf(cur
                                                                                                                + dist.doubleValue()));
                                ...
                                .flatMap(trip -> {
                                        ...
                                        return driverPersistence
                                                        ...
                                                        .flatMap(driver -> {
                                                                ...
                                                                return tripRepository
                                                                                .save(trip)
                                                                                .flatMap(saved -> loadTripWithDetails(
                                                                                                saved.getId()));
                                                        });
                                });
        }
        ...
        public Flux<Trip> getManagerTrips(UUID managerId, UUID fleetId) {
                // G9 FIX: Tentative d'abord par organisation (Admin voit tout)
                Flux<TripEntity> source;
                if (fleetId != null) {
                        source = tripRepository.findAllByCreatedByAndFleetId(managerId, fleetId);
                } else {
                        // D'abord essayer par createdBy (Manager), sinon par organisation (Admin)
                        source = tripRepository.findAllByCreatedBy(managerId)
                                        .switchIfEmpty(tripRepository.findAllBySameCompanyAsUser(managerId));
                }
                ...
                return tripRepository
                                ...
                                .flatMap(trip -> {
                                        ...
                                        Mono<Void> sqlTask = operationalRepo
                                                        ...
                                                        .flatMap(op -> {
                                                                op.setCurrentSpeed(
                                                                                BigDecimal.valueOf(speed != null ? speed
                                                                                                : 0.0));
                                ...
                                .flatMap(trip -> {
                                        if (!"SCHEDULED".equals(trip.getStatus())) {
                                                return Mono.error(new RuntimeException(
                                                                "Trip cannot be started in its current status: "
                                                                                + trip.getStatus()));
                                        }
                ...
                return tripRepository.findById(tripId)
                                ...
                                .flatMap(trip -> {
                                        if (!"DEPARTED".equals(trip.getStatus())) {
                                                return Mono.error(new RuntimeException(
                                                                "Trip cannot transition to returning from status: "
                                                                                + trip.getStatus()));
                                        }
