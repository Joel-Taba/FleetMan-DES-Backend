...
public interface ManageDriverUseCase {
        ...
        Mono<Driver> updateDriver(UUID userId, UpdateDriverRequest request, UUID requesterId, String token);

        // --- AJOUTS TACHE 8 ---
        Mono<Driver> registerDriverWithPhoto(UUID fleetId, DriverRegistrationRequest request, UUID managerId,
                        AuthUseCase.FileContent photo);

        Flux<Driver> getDriversWithFilters(UUID fleetId, Boolean isAssigned, UUID requesterId);

        Flux<com.yowyob.fleet.infrastructure.adapters.inbound.rest.dto.DriverResponse> getDriversEnriched(UUID fleetId,
                        Boolean isAssigned, UUID requesterId);
        ...
        Mono<Void> unassignSpecificVehicle(UUID userId, UUID vehicleId, UUID requesterId);
