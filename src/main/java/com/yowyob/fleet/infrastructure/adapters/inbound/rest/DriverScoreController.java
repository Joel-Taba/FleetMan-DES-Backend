...
public class DriverScoreController {
    ...
    @Operation(summary = "Dernier score d'un chauffeur", description = "Retourne le score le plus récent d'un chauffeur. Déclenche un calcul automatique si aucun score n'existe encore pour la période courante.")
    public Mono<DriverScoreResponse> getLatestScore(
            @Parameter(description = "ID du chauffeur") @PathVariable UUID driverId,
            @Parameter(description = "Période (WEEKLY ou MONTHLY)", example = "MONTHLY") @RequestParam(defaultValue = "MONTHLY") DriverScore.PeriodType periodType) {
    ...
    @PreAuthorize("hasAnyRole('FLEET_MANAGER', 'FLEET_DRIVER')")
    @Operation(summary = "Historique des scores d'un chauffeur", description = "Retourne l'évolution du score sur une plage de dates. Idéal pour afficher un graphique de tendance.")
    public Flux<DriverScoreResponse> getHistory(
            @Parameter(description = "ID du chauffeur") @PathVariable UUID driverId,
            @Parameter(description = "Période (WEEKLY ou MONTHLY)", example = "MONTHLY") @RequestParam(defaultValue = "MONTHLY") DriverScore.PeriodType periodType,
            @Parameter(description = "Date de début", example = "2026-01-01") @RequestParam(required = false) LocalDate from,
            @Parameter(description = "Date de fin", example = "2026-06-30") @RequestParam(required = false) LocalDate to) {
        LocalDate start = from != null ? from : LocalDate.now().minusDays(30);
        LocalDate end = to != null ? to : LocalDate.now();
        return scoringUseCase.getScoreHistory(driverId, periodType, start, end)
    ...
    @PreAuthorize("hasRole('FLEET_MANAGER')")
    @Operation(summary = "Scores de tous les chauffeurs d'une flotte", description = "Retourne le score de la période courante pour tous les chauffeurs de la flotte, triés par score décroissant.")
    ...
    @PreAuthorize("hasRole('FLEET_MANAGER')")
    @Operation(summary = "Classement des meilleurs chauffeurs", description = "Retourne les N meilleurs chauffeurs d'une flotte selon leur score de la période courante.")
    public Flux<DriverScoreResponse> getTopDrivers(
            @Parameter(description = "ID de la flotte") @PathVariable UUID fleetId,
            @Parameter(description = "Période", example = "MONTHLY") @RequestParam(defaultValue = "MONTHLY") DriverScore.PeriodType periodType,
            @Parameter(description = "Nombre de chauffeurs à retourner (1-100)", example = "5") @RequestParam(defaultValue = "5") int limit) {
    ...
    @PreAuthorize("hasRole('FLEET_MANAGER')")
    @Operation(summary = "Chauffeurs les plus à risque", description = "Retourne les N chauffeurs avec les scores les plus bas. Idéal pour cibler les actions correctives.")
    ...
    @PreAuthorize("hasRole('FLEET_MANAGER')")
    @Operation(summary = "Résumé des scores d'une flotte", description = "Distribution des badges, score moyen/min/max. Utilisé pour le dashboard Manager.")
    ...
    @PreAuthorize("hasRole('FLEET_MANAGER')")
    @Operation(summary = "Calculer le score d'un chauffeur", description = "Force le recalcul du score d'un chauffeur pour une période spécifique. "
            +
            "Remplace le score existant si présent.")
    public Mono<DriverScoreResponse> calculateScore(
            @Parameter(description = "ID du chauffeur") @PathVariable UUID driverId,
            @Parameter(description = "Période", example = "MONTHLY") @RequestParam(defaultValue = "MONTHLY") DriverScore.PeriodType periodType,
            @Parameter(description = "Début de la période (YYYY-MM-DD, défaut : début du mois courant)") @RequestParam(required = false) LocalDate periodStart) {
    ...
    @PreAuthorize("hasRole('FLEET_MANAGER')")
    @Operation(summary = "Calculer les scores de toute une flotte", description = "Déclenche le recalcul des scores pour tous les chauffeurs actifs de la flotte.")
    public Flux<DriverScoreResponse> calculateFleetScores(
            @Parameter(description = "ID de la flotte") @PathVariable UUID fleetId,
            @Parameter(description = "Période", example = "MONTHLY") @RequestParam(defaultValue = "MONTHLY") DriverScore.PeriodType periodType,
            @Parameter(description = "Début de la période (YYYY-MM-DD)") @RequestParam(required = false) LocalDate periodStart) {
