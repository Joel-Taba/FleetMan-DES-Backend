...
public class KpiController {
        ...
        @Operation(summary = "KPIs d'une flotte", description = "Retourne les KPIs les plus récents pour une flotte. "
                        + "Si aucun snapshot n'existe, déclenche un calcul à la demande.")
        public Mono<KpiSnapshot> getFleetKpi(
                        @PathVariable UUID fleetId,
                        @Parameter(description = "Granularité") @RequestParam(defaultValue = "MONTHLY") String period) {
        ...
        @PreAuthorize("hasAnyRole('FLEET_MANAGER', 'FLEET_ADMIN', 'FLEET_SUPER_ADMIN')")
        @Operation(summary = "Historique des KPIs d'une flotte", description = "Retourne l'évolution des KPIs sur une plage de dates.")
        public Mono<List<KpiSnapshot>> getFleetKpiHistory(
                        @PathVariable UUID fleetId,
                        @RequestParam(defaultValue = "MONTHLY") String period,
                        @Parameter(example = "2026-01-01") @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate from,
                        @Parameter(example = "2026-06-30") @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate to) {
        ...
        @PreAuthorize("hasAnyRole('FLEET_MANAGER', 'FLEET_ADMIN', 'FLEET_SUPER_ADMIN')")
        @Operation(summary = "Comparer deux périodes", description = "Compare les KPIs de deux périodes identiques. "
                        + "Retourne les deltas absolus et en pourcentage.")
        public Mono<KpiUseCase.KpiComparisonDto> compareFleetKpi(
                        @PathVariable UUID fleetId,
                        @RequestParam(defaultValue = "MONTHLY") String period,
                        @Parameter(description = "Début période 1", example = "2026-04-01") @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate period1Start,
                        @Parameter(description = "Début période 2", example = "2026-05-01") @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate period2Start) {
        ...
        @PreAuthorize("hasAnyRole('FLEET_MANAGER', 'FLEET_ADMIN', 'FLEET_SUPER_ADMIN')")
        @Operation(summary = "Top véhicules par kilométrage", description = "Retourne les N véhicules les plus actifs de la flotte.")
        public Mono<List<KpiSnapshot>> getTopVehicles(
                        @PathVariable UUID fleetId,
                        @RequestParam(defaultValue = "MONTHLY") String period,
                        @Parameter(description = "Nombre de résultats", example = "5") @RequestParam(defaultValue = "5") int limit) {
        ...
        @PreAuthorize("hasAnyRole('FLEET_MANAGER', 'FLEET_ADMIN', 'FLEET_SUPER_ADMIN')")
        @Operation(summary = "Top conducteurs par score", description = "Retourne les N meilleurs conducteurs selon leur score de conduite.")
        ...
        @PreAuthorize("hasAnyRole('FLEET_MANAGER', 'FLEET_ADMIN', 'FLEET_SUPER_ADMIN')")
        @Operation(summary = "Exporter les KPIs en CSV", description = "Génère un fichier CSV de l'historique des KPIs. "
        ...
        public Mono<ResponseEntity<String>> exportCsv(
                ...
                return CsvExportUtil.export(
                                ...
                                s -> List.of(
                                                ...
                                                s.totalMaintenanceCost() != null
                                                                ? s.totalMaintenanceCost().toPlainString()
                                                                : "0",
                                                s.totalIncidentCost() != null ? s.totalIncidentCost().toPlainString()
                                                                : "0",
                                                ...
                                                s.docComplianceRate() != null ? s
                                                                .docComplianceRate().toPlainString() : ""))
                                .map(csv -> ResponseEntity.ok()
                                                .header(HttpHeaders.CONTENT_DISPOSITION,
                                                                "attachment; filename=\"kpis-" + fleetId + "-" + period
                                                                                + ".csv\"")
        ...
        @Operation(summary = "Recalculer les KPIs (Admin)", description = "Déclenche un recalcul immédiat des KPIs pour une flotte. "
                        + "Réservé aux administrateurs.")
        public Mono<KpiSnapshot> recalculate(
                        @PathVariable UUID fleetId,
                        @RequestParam(defaultValue = "MONTHLY") String period,
                        @Parameter(example = "2026-05-01") @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate periodStart) {
