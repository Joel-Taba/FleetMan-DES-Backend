...
public class MaintenanceController {
    ...
    @Operation(summary = "Déclarer une maintenance", description = "Enregistre une intervention technique sur un véhicule. Notifie automatiquement le Fleet Manager.")
    public Mono<MaintenanceResponse> create(
            @Valid @RequestBody MaintenanceRequest request) {

        ManageMaintenanceUseCase.CreateMaintenanceCommand cmd = new ManageMaintenanceUseCase.CreateMaintenanceCommand(
                request.subject(),
                request.cost(),
                request.report(),
                request.longitude(),
    ...
    @Operation(summary = "Lister mes maintenances", description = "Retourne toutes les maintenances des véhicules gérés par le manager connecté.")
    ...
    @PreAuthorize("hasRole('FLEET_MANAGER')")
    @Operation(summary = "Maintenances d'un véhicule", description = "Retourne l'historique complet des maintenances pour un véhicule donné.")
    ...
    @PreAuthorize("hasAnyRole('FLEET_MANAGER', 'FLEET_DRIVER')")
    @Operation(summary = "Maintenances d'un chauffeur", description = "Retourne toutes les maintenances impliquant un chauffeur donné.")
    ...
    @PutMapping("/{id}")
    @PreAuthorize("hasRole('FLEET_MANAGER')")
    @Operation(summary = "Mettre à jour une maintenance", description = "Met à jour le rapport, le coût ou la localisation d'une maintenance existante.")
    public Mono<MaintenanceResponse> update(
            @Parameter(description = "ID de la maintenance") @PathVariable UUID id,
            @Valid @RequestBody MaintenanceRequest request) {

        ManageMaintenanceUseCase.UpdateMaintenanceCommand cmd = new ManageMaintenanceUseCase.UpdateMaintenanceCommand(
                id,
                request.subject(),
                request.cost(),
                request.report(),
