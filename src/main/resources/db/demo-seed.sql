-- Données de démonstration FleetMan (profil demo / auth fake)
-- UUID fixes — voir DemoTestAccounts.java et IDENTIFIANTS_TEST.md

-- 1. Utilisateurs locaux
INSERT INTO fleet.users (id, username, email, first_name, last_name, is_active)
VALUES
...
SELECT 'c0000001-0000-4000-8000-000000000001', 'b0000001-0000-4000-8000-000000000001', 'a0000003-0000-4000-8000-000000000003',
       'LT-892-CE', 'Toyota', 'Hilux', 2022, 'Bleu', 'ON_TRIP', vt.id
FROM fleet.vehicle_types vt WHERE vt.code = 'TRUCK' LIMIT 1
ON CONFLICT (id) DO NOTHING;

INSERT INTO fleet.vehicles (id, fleet_id, manager_id, license_plate, brand, model, manufacturing_year, color, status, vehicle_type_id)
SELECT 'c0000002-0000-4000-8000-000000000002', 'b0000002-0000-4000-8000-000000000002', 'a0000003-0000-4000-8000-000000000003',
       'CE-456-AB', 'Mercedes', 'Actros', 2020, 'Blanc', 'AVAILABLE', vt.id
FROM fleet.vehicle_types vt WHERE vt.code = 'TRUCK' LIMIT 1
ON CONFLICT (id) DO NOTHING;

INSERT INTO fleet.vehicles (id, fleet_id, manager_id, license_plate, brand, model, manufacturing_year, color, status, vehicle_type_id)
SELECT 'c0000003-0000-4000-8000-000000000003', 'b0000001-0000-4000-8000-000000000001', 'a0000003-0000-4000-8000-000000000003',
...
VALUES ('a0000004-0000-4000-8000-000000000004', 'b0000001-0000-4000-8000-000000000001', 'B1234567CM', 'ACTIVE', 'c0000001-0000-4000-8000-000000000001')
ON CONFLICT (user_id) DO NOTHING;
...
-- 6. Trajets
INSERT INTO fleet.trips (id, vehicle_id, driver_id, start_date, end_date, start_time, end_time, status, distance_km, duration_minutes)
VALUES
...
VALUES
  ('e0000001-0000-4000-8000-000000000001', 'a0000003-0000-4000-8000-000000000003', 'b0000001-0000-4000-8000-000000000001', 'CIRCLE'),
  ('e0000002-0000-4000-8000-000000000002', 'a0000003-0000-4000-8000-000000000003', 'b0000002-0000-4000-8000-000000000002', 'CIRCLE')
ON CONFLICT (id) DO NOTHING;
...
VALUES ('f1000001-0000-4000-8000-000000000001', 'Vidange + filtres', 85000, 'c0000003-0000-4000-8000-000000000003', 'SW-123-DL', 'a0000004-0000-4000-8000-000000000004', 'André Mbarga')
ON CONFLICT (id) DO NOTHING;
...
VALUES ('f1000002-0000-4000-8000-000000000002', 'ACCIDENT', 'Rayure latérale parking', 'MEDIUM', 'c0000001-0000-4000-8000-000000000001', 'LT-892-CE', 'a0000004-0000-4000-8000-000000000004', 'André Mbarga', 'REPORTED')
ON CONFLICT (id) DO NOTHING;
...
VALUES ('f1000003-0000-4000-8000-000000000003', 65, 42250, 'TOTAL', 'c0000001-0000-4000-8000-000000000001', 'LT-892-CE', 'a0000004-0000-4000-8000-000000000004', 'André Mbarga')
ON CONFLICT (id) DO NOTHING;
...
VALUES ('f2000003-0000-4000-8000-000000000003', 'a0000004-0000-4000-8000-000000000004', 'DRIVING_LICENSE', 'B1234567CM', '2027-03-10', 'VALID')
ON CONFLICT (id) DO NOTHING;
...
VALUES
  ...
  ('f3000004-0000-4000-8000-000000000004', 'f3000001-0000-4000-8000-000000000001', 'b0000001-0000-4000-8000-000000000001',
   'c0000003-0000-4000-8000-000000000003', 'a0000004-0000-4000-8000-000000000004',
...
VALUES ('f5000001-0000-4000-8000-000000000001', 'FLEET', 'b0000001-0000-4000-8000-000000000001',
        'a0000003-0000-4000-8000-000000000003', '2026-06-01', 2500000, 422500, 'NORMAL')
ON CONFLICT (id) DO NOTHING;
...
VALUES ('f6000001-0000-4000-8000-000000000001', 'a0000004-0000-4000-8000-000000000004', 'b0000001-0000-4000-8000-000000000001',
        'a0000003-0000-4000-8000-000000000003', 'MONTHLY', '2026-06-01', '2026-06-30', 48, 92, 'EXCELLENCE')
ON CONFLICT (id) DO NOTHING;

-- 14. Notifications
INSERT INTO fleet.notification_settings (user_id, enable_email, enable_push)
VALUES
  ('a0000003-0000-4000-8000-000000000003', true, true),
  ('a0000004-0000-4000-8000-000000000004', true, true)
ON CONFLICT (user_id) DO NOTHING;

INSERT INTO fleet.notifications (id, user_id, title, message, type, is_read)
VALUES
  ('f7000001-0000-4000-8000-000000000001', 'a0000003-0000-4000-8000-000000000003', 'Incident signalé', 'Rayure véhicule LT-892-CE', 'WARNING', false),
  ('f7000002-0000-4000-8000-000000000002', 'a0000004-0000-4000-8000-000000000004', 'Affectation confirmée', 'Créneau demain 14h-18h', 'INFO', false)
ON CONFLICT (id) DO NOTHING;
